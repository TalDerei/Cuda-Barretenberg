...
ec_multiexp_straus(var *out, const var *multiples_, const var *scalars_, size_t N)
{
    ...
    if (idx < n) {
        ...
        // scalars stored in registers
        Fr scalars[RR];
        for (int j = 0; j < R; ++j) {
            // sequential access across threads 
...
ec_reduce_straus(cudaStream_t &strm, var *out, const var *multiples, const var *scalars, size_t N)
{
    ...
    for ( ; m != 0; r = m & 1, m >>= 1) {
        ...
        //synchronize -- return cudaerrortype
        if (r)
...
var_ptr
allocate_memory(size_t nbytes, int dbg = 1) {
    var *mem = nullptr;
...
{
    // Allocating space for n * scalar_bytes (96 bytes) of space
    static constexpr size_t scalar_bytes = ELT_BYTES;
    size_t total_bytes = n * scalar_bytes;
    printf("size of total_bytes is: %zu\n", total_bytes);
...
{
    // Field type of the elliptic curve
    typedef typename EC::field_type FF;

    // 96 bytes per element
    static constexpr size_t coord_bytes = FF::DEGREE * ELT_BYTES;

    // Affine points are a pair of elements, so it's 2 * 96 = 192 bytes per affine point
    static constexpr size_t aff_pt_bytes = 2 * coord_bytes;

    printf("Size of n is: %zu\n", n);

    // Total number of affine points is the size of the input
    size_t total_aff_bytes = n * aff_pt_bytes;
    printf("total_aff_bytes is: %zu\n", total_aff_bytes);

    // Allocate memory of size 'total_aff_bytes' using cudaMallocManaged
    auto mem = allocate_memory(total_aff_bytes, 1);
    if (fread((void *)mem.get(), total_aff_bytes, 1, inputs) < 1) {
        fprintf(stderr, "Failed to read all curve poinst\n");
        abort();
    }
    // Allocated 195041088 bytes; device has 22304.1 MiB free (98.8%). Returning a unique_ptr
    return mem;
}
